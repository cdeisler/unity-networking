...
namespace UnityEngine.Networking
{
    public class ClientScene
    {
        ...
        #region ----- Manager PlayerController instances -----
        ...
        else
        {
            ...
            if (s_Peers == null)
            {
                ...
                if (LogFilter.logError) { Debug.LogError("ClientScene::AddPlayer: reconnecting, but no peers."); }
                return false;
            }
        ...
        #region ------------------------ Object lifetime management messages ---------------------
        ...
        static void OnObjectSpawn(NetworkMessage netMsg)
        {
            ...
            {
                if (LogFilter.logError) { Debug.LogError("Failed to spawn server object, assetId=" + s_ObjectSpawnMessage.assetId + " netId=" + s_ObjectSpawnMessage.netId); }
            }
        }
        ...
        static void OnRPCMessage(NetworkMessage netMsg)
        {
            ...
            {
                if (LogFilter.logWarn) { Debug.LogWarning("Did not find target for RPC message for " + netId); }
            }
        }
